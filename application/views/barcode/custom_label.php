  ...
  foreach ($products as $lab) {
      for ($i = 0; $i <= $style['total_rows']; $i++) {
          for ($z = 0; $z <= $style['items_per_row']; $z++) {
              ...
              if ($style['product_name']) {
                  echo substr($lab['product_name'], 0, $style['max_char']);
              }
              if ($style['product_code']) {
                  echo  '<br>'.$lab['product_code'] ;
              }
              ...
              echo  $lab['barcode'].'<br>' ;
              if ($style['product_price']) {
                  echo '<strong>'.amountExchange($lab['product_price'], 0, $this->aauth->get_user()->loc) . ' </strong><br>';
              }
              if ($style['store_name']) {
                  echo  substr($style['store'], 0, $style['max_char']).'<br>' ;
              }
              if ($style['warehouse_name']) {
                  echo    substr($style['warehouse'], 0, $style['max_char']);
              }
              if ($lab['expiry']) {
                  echo '<br>'.$this->lang->line('Expiry Date') . ' ' . dateformat($lab['expiry']) ;
              }
              echo'</div>';
          }
      }
